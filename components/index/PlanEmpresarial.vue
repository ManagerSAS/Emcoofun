<template>
    <v-container fluid class="mb-3"  justify="center" align="center">
        <v-row  justify="center" align="center" class="mt-5 ml-8 mr-5">
            <v-col cols="12" sm="12" md="11" lg="8" justify="start" align="center" >
                <h2 class="text-raleway font-weight-bold" style="color: #E83E58; font-size: 40px;"> PLAN EXEQUIAL EMPRESARIAL </h2>
            </v-col>
        </v-row>
        <v-row  class="ml-lg-15 ml-8 mr-5" justify="center" align="center">
            <v-col cols="12" sm="10" md="6" lg="5" xl="5">
                <div class="mb-6 ml-md-5">
                    <h4 class="text-raleway " style="font-size: 40px; color: #003B4C; font-weight: 500;">En los Olivos</h4>
                    <h4 class="text-raleway " style="font-size: 30px; color: #003B4C; font-weight: 900;">nos importa la integridad y bienestar </h4>
                    <h4 class="text-raleway " style="font-size: 30px; color: #003B4C;">de cada integrante de tu organización  </h4>
                </div>
                    <v-btn
                        style="background: linear-gradient(to right, #EC6480, #E83E58)" 
                        class="shadow-principal btn-hover ml-md-5 text-raleway "
                        dark
                        rounded-xl
                        href="https://api.whatsapp.com/send?phone=573174041842&text=Hola%20%F0%9F%96%90%EF%B8%8F%2C%20necesito%20un%20asesor%20empresarial%20de%20Emcoofun%20Los%20Olivos%20por%20favor%20%F0%9F%99%8F%F0%9F%98%80"
                    >
                         CONOCE NUESTRO PLAN
                    </v-btn>
            </v-col>
            <v-col justify="center" align="center" cols="12" sm="12" md="6" lg="5" xl="5">
                <img class="d-none d-xl-block d-lg-block" src="../../static/planEmpresa.png" width="600">
                <img class="d-none d-md-block d-lg-none" src="../../static/planEmpresa.png" width="450">
                <img class="d-block d-sm-none" src="../../static/planEmpresa.png" width="300">
            </v-col>
        </v-row>
    </v-container >

</template>
